...
var todoTemplate = function(todo) {
    ...
    var t = `
        <div class="todo-cell">
            <span>${todo.task}</span>
			<button class="todo-delete pure-button" todo_id='${todo.id}'>删除</button>
			<button class="todo-edit pure-button" todo_id='${todo.id}'>编辑</button>
			<br>
			${str_time}
        </div>
    `
	...
	// 点击编辑按钮后，弹出更新内容提交
	 todoList.addEventListener('click', function(event){
		 ...
		 if (self.classList.contains('todo-edit')) {
			 ...
			 var update_form = `
					<br>
					<input id='id-input-todo' todo_id=${todo_id}>
					<button id='id-button-add' class="todo-update pure-button pure-button-primary">更新</button>
	 ...
	 todoList.addEventListener('click', function(event){
		 ...
		 if (self.classList.contains('todo-update')) {
			 ...
			 apiTodoUpdate(form, function(r) {	
				log(r)
				var todo = JSON.parse(r)
				var a = self.parentNode.getElementsByTagName('span')[0]
